...
if share:
    grm_cols = list(grm_df.columns.values)
    assert set(grm_cols) == set(new_cols)
    grm_col_map = {i:j for i,j in enumerate(grm_cols)}
    new_grm_cols = [grm_col_map[i] for i in grm_cols]
    new_prm_cols = [grm_col_map[i] for i in new_cols]
    grm_df.columns = new_grm_cols
    prm_df.columns = new_prm_cols
    new_proband1 = [grm_col_map[str(i)] for i in relative_df.proband1]
    new_proband2 = [grm_col_map[str(i)] for i in relative_df.proband2]
    relative_df['proband1'] = new_proband1
    relative_df['proband2'] = new_proband2
